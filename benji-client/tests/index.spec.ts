...
test.beforeAll(async () => {
  ...
  await page.goto('https://discord.com/api/oauth2/authorize?client_id=712958072007688232&permissions=8&redirect_uri=http%3A%2F%2Flocalhost%3A3000/login&response_type=code&scope=identify%20guilds');
  ...
  await expect(page.locator('.InactiveGuild_prompt__zovlt > h1')).toHaveText('Select a server to start!');
...
test('Add song and check that count increases', async ({ browser }) => {
  const context = await browser.newContext({ storageState: 'tests/state.json' });
  const page = await context.newPage();
  ...
  await page.waitForTimeout(500);
  const newQueueLength = await page.locator('.Queue_queue__item__1KnmH').count();
